...
public class DeckStrategyFoodEffectsAndAttributes implements DeckStrategy {
    private final RandomSelector randomSelector;
    public DeckStrategyFoodEffectsAndAttributes(RandomSelector randomSelector){this.randomSelector = randomSelector; }
    @Override
    public ArrayList<Card> giveDeck(Player player) {
        ...
        int random_two_mana = randomSelector.getRandomNumber(9);
        int random_four_mana = randomSelector.getRandomNumber(14);
        ...
        initialDeck.add(new StandardCard(GameConstants.TOMATO_SALAD_CARD, 2, 3, 2, player, new EffectGiveAttackToRandomMinion(randomSelector,1, player, player)));
        initialDeck.add(new StandardCard(GameConstants.SHRIMP_COCKTAIL_CARD, 2, 2, 2, player, CardAttribute.TAUNT));
        initialDeck.add(new StandardCard(GameConstants.SHRIMP_COCKTAIL_CARD, 2, 2, 2, player, CardAttribute.TAUNT));
        initialDeck.add(new StandardCard(GameConstants.POKE_BOWL_CARD, 3, 2, 3, player, new EffectGiveTwoHealthHero()));
        initialDeck.add(new StandardCard(GameConstants.POKE_BOWL_CARD, 3, 2, 3, player, new EffectGiveTwoHealthHero()));
        initialDeck.add(new StandardCard(GameConstants.TZATZIKI_CARD, 3, 3, 2, player, CardAttribute.TAUNT));
        initialDeck.add(new StandardCard(GameConstants.TZATZIKI_CARD, 3, 3, 2, player, CardAttribute.TAUNT));
        ...
        initialDeck.add(new StandardCard(GameConstants.BAKED_SALMON_CARD, 5, 8, 2, player));
        initialDeck.add(new StandardCard(GameConstants.LASAGNA_CARD, 5, 2, 5, player, CardAttribute.TAUNT));
        initialDeck.add(new StandardCard(GameConstants.LASAGNA_CARD, 5, 2, 5, player, CardAttribute.TAUNT));
        ...
        if (player == Player.PEDDERSEN)
            finalDeck.add(0, new StandardCard(GameConstants.MUSLI_BAR_CARD, 0, 0, 0, player, new EffectPlusMana(1)));
