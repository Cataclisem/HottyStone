...
public class HotStoneDrawing implements Drawing, GameObserver {
  ...
  // methods instead!
  private Map<String, HotStoneActorFigure> actorMap;
  ...
  private void createAndAddFiguresForGameState() {
    ...
    TextFigure myHeroPowerText = new TextFigure(hero.getPowerDesc(),
    ...
    TextFigure oppHeroPowerText = new TextFigure(hero.getPowerDesc(),
  ...
  public void onCardPlay(Player who, Card card) {
    ...
    if (card.getCardEffectDescription() != "Does nothing")
      addMessage("Using effect " + card.getCardEffectDescription());
  ...
  public void onAttackHero(Player playerAttacking, Card attackingCard) {
    Player opp = Utility.computeOpponent(playerAttacking);
    addMessage(""+ playerAttacking
            + " attacks Peddersen with "
            + attackingCard.getName() + ", he loses "
            + attackingCard.getAttack()
            + " and has "
            + game.getHero(opp).getHealth() + " left!");
  }

  @Override
  public void onUsePower(Player who) {
    addMessage(who + " used Hero Power " + game.getHero(who).getPowerDesc());
  }
  ...
  public void onCardUpdate(Card card) {
    HotStoneActorFigure actor = actorMap.get(card.getID());
    // Opponent cards may not have an associated actor
    // for instance if they are in the hand.
    if (actor != null) {
      // updated the stats of the card/minion
      actor.updateStats();
      addMessage("Updated " + card.getName() + " stats");
    }
  }
  ...
  public void onGameWon(Player playerWinning) {
    haltOnGameWonScreen(playerWinning);
  }
  ...
  private void refreshField(Player who) {
    ...
    // Central parameters for the layout
  ...
  private void createActorAndUpdateMapping(Card card, HotStoneFigureType type) {
    ...
    if (type == HotStoneFigureType.CARD_FIGURE) {
      actor = new CardFigure(card, new Point(0, 500)); // Will be laid out later
    } else {
   ...
   */
  private void  haltOnGameWonScreen(Player playerwinning) {
    removeAllFigures();

    String winner;
    // Show game as a draw if no one wins
    if (playerwinning == null){
      winner = "Game is a draw.";
    }
    // Otherwise give the winner
    else {
      winner = "Game won by " + playerwinning.toString() + ".";
    }
